...
try {
    $mdata = $DB->get_records('modules', array('name' => 'kalvidres'));
    ...
    foreach ($mdata as $row1) {
        ...
        $query .= 'where component=\'mod_kalvidres\' and action=\'viewed\' and contextinstanceid=:cmid1 ';
        ...
        $query .= 'from {logstore_standard_log} ';
        $query .= 'where component=\'mod_kalvidres\' and action=\'played\' and contextinstanceid=:cmid2 ';
        ...
        foreach ($logdata as $row2) {
            if (! $kalvidreslog = $DB->get_record('kalvidres_log',
                ...
                $DB->insert_record('kalvidres_log', $dataobject, true, false);
                $skip = false;
            }
        }
        unset($logdata);
