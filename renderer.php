...
class mod_kalvidres_renderer extends plugin_renderer_base {
    ...
    public function embed_media($kalvidres) {
        ...
        $entry_obj = \local_kaltura\kaltura_entry_manager::get_entry($client, $kalvidres->entry_id, false);
        $client->session->end();
        if (!$entry_obj->totalCount) {
            $client_legacy = \local_kaltura\kaltura_client::get_client('ce', 'admin');
            $entry_obj = \local_kaltura\kaltura_entry_manager::get_entry($client_legacy, $kalvidres->entry_id, false);
        ...
        } else if ($entry_obj->objects[0]->status != \KalturaEntryStatus::READY) {
